...
public:
  ...
  // type: 
  // 0: ÉGÉbÉWÇÃÉAÉbÉvÉfÅ[ÉgÇÃÇ∆Ç´
  // 1: ä»ó™âªÇ…î∫Ç§è¡ãéÇÃÇ∆Ç´
  void clear( int type ) {
    sn_ = NULL;
    en_ = NULL;
    setIsDeleted( true );
	...
	// PIEM ÇÕÇ±Ç±Ç≈è¡ãé
	deletePIEM();
	// Ç±Ç±Ç…Ç†ÇÈ slimball ÇÕè¡ãé
	deleteSlimBall();
	break;
      case 1: // Ç±Ç±Ç≈ÇÕâΩÇ‡è¡ãéÇµÇ»Ç¢ (Node Ç…à¯Ç´åpÇ™ÇÍÇƒÇ¢ÇÈÇÕÇ∏)
...
public:
  ...
  // è¡ãéÇÕä»ó™âªÇÃé¿çsÇÃÇ∆Ç´ÇÃÇ›
  void clear() {
    face_list_.clear();
    edges_.clear();
    setIsDeleted( true );

    // slimball ÇÃï˚ÇÕè¡ãéÇµÇ»Ç¢
    slimball_ = NULL;
  ...
  void add( DGNode* node, DGEdge* orig ) {
	...
	// edge ÇÃóºí[ÇÃÉmÅ[ÉhÇÃÇ§Çø, en -> sn Ç∆Ç∑ÇÈ
	...
	for ( int j = 0; j < edges_.size(); ++j )
	  {
	    ...
	    if ( ((edges0->sn() == edges1->sn()) && (edges0->en() == edges1->en())) ||
		 ((edges0->sn() == edges1->en()) && (edges0->en() == edges1->sn())) )
	      {
		// edge1 Ç™ crease ÇæÇ¡ÇΩÇÁÅCedge0 Ç…à¯Ç´åpÇÆ
...
public:
  ...
  // id / 3 = id0
  //
  // MeshRDG ÇÃóvëfÇ÷ÇÕ 1/3 ÇµÇƒäiî[Ç∑ÇÈ
  //
  void createDG() {
...
// 	m[2] = (int) ((float) face_mates[i+2] / 3.0f);
...
#if 0
		if ( mesh().dihedralAngle( id, m[j] ) > CREASE_ANGLE ) // 45Åã
		  {
  ...
  //
  // èkëﬁÇµÇΩñ ÇÃÉmÅ[Éh (DGNode) ÇçÌèú
  //
  void checkDG() {
	...
	// Mesh ÇÃñ ÇÕàÍÇ¬ÇµÇ©ìoò^Ç≥ÇÍÇƒÇ¢Ç»Ç¢ÇÕÇ∏
	std::vector<unsigned int>& face_list = node->face_list();
...
// 	cout << "bb" << endl;
	...
	// èkëﬁÇµÇƒÇ¢ÇÈèÍçá, ÉÅÉCÉgÇÃñ ìØémÇÇ≠Ç¡Ç¬ÇØÇÈ
// 	cout << "aa" << endl;
